...
module M = {
  ...
  proc chacha20_more_than_256 (output:W64.t, plain:W64.t, len:W32.t,
    ...
    s_plain <- plain;
    s_output <- output;
    if (((W32.of_int 512) \ule len)) {
      ____UNIT____ <- LFENCE ;
      plain <- ((len \ult (W32.of_int 512)) ? s_plain : plain);
      output <- ((len \ult (W32.of_int 512)) ? s_output : output);
      len <- ((len \ult (W32.of_int 512)) ? (truncateu32 r512) : len);
      k <@ copy_state_x8 (st);
  ...
  proc chacha20_avx2_v4_safe_movcc (output:W64.t, plain:W64.t, len:W32.t,
                                    key:W64.t, nonce:W64.t, counter:W32.t) : unit = {
    var ____UNIT____ : unit;
